...
public class SessionPricedEventHandler : IRequestHandler<SessionPriced>
{
    ...
    public SessionPricedEventHandler(IOptions<SessionOrchestratorServiceConfig> config, ILogger<SessionPricedEventHandler> logger)
    {
        _logger = logger;
    ...
    public async Task Handle(SessionPriced request, CancellationToken cancellationToken)
    {
        ...
        var pricingUpdateRequest = new PricingUpdateRequest(request.SessionId, request.Price, request.PriceAfterTax, request.TaxBasisPoints, request.TaxAmount);
